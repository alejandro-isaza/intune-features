...
public class Microphone {
    ...
    public func start() {
        if running { return }
        checkStatus(AudioOutputUnitStart(inputUnit), "AudioComponentInstanceDispose")
        running = true
    }

    public func stop() {
        if !running { return }
        checkStatus(AudioOutputUnitStop(inputUnit), "AudioComponentInstanceDispose")
        running = false
    }
    ...
    private func create() {
        ...
        checkStatus(AudioUnitSetProperty(inputUnit, kAudioUnitProperty_MaximumFramesPerSlice, kAudioUnitScope_Global, outputBus, &maxFrames, UInt32(sizeof(maxFrames.dynamicType))), "AudioUnitSetProperty buffer output")
    }
