...
public class Table {
    ...
    public init(file: File, name: String, rowSize: Int, chunkSize: Int = 1024) {
        ...
        if let dataset = file.openDataset(name, type: Double.self) {
    ...
    func verifyDataset(dataset: Dataset<Double>) {
    ...
    func createDataset() {
        ...
        file.createDataset(name, type: Double.self, dataspace: space, chunkDimensions: [chunkSize, rowSize])
    }
    ...
    public func readFromRow(start: Int, count: Int, into pointer: UnsafeMutablePointer<Double>) throws -> Int {
        ...
        if !dataset.readDouble(pointer, memSpace: memSpace, fileSpace: fileSpace) {
            throw Error.IOError
        }
